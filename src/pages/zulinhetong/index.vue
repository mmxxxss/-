...
import { getRentContractList } from "../../api/zuke";
import { ref } from "vue";
// 组件参数
const roomList = ref([]);
// 组件方法
const getRoomListData = async () => {
  const res = await getRentContractList();
  if (res.code == 0) {
    roomList.value = res.data.list;
  }
};
const openYuYueKanFangDetail = (item) => {
  Taro.navigateTo({
    url: "/pages/zulinhetongdetail/index?id=" + item.id,
  });
};
getRoomListData();
</script>
<template>
  <div class="container">
    <div
      ...
      :key="index"
      class="item"
      @click="openYuYueKanFangDetail(item)"
    >
      <image
        :src="
